...
class matrixTree extends Component
{
    ...
    private function buildTree(User $user, int $level = 0): array
    {
        ...
        $branch = [
            ...
            'children' => [],
            'direct_affiliates' => $user->matrixTotal?->current_affiliates ?? 0,
            'total_affiliates' => $user->matrixTotal?->total_affiliates ?? 0,
        ];
    ...
    public function show(User $user): void
    {
        ...
        if ($this->currentUser->id === $this->secondaryUserId) {
            $this->currentUser = User::findOrFail($user->matrix->placement_id);
            $this->secondaryUserId = $this->currentUser->id;
        } else {
            $this->secondaryUserId = $this->currentUser->id;
        }
    ...
    public function render()
    {
        ...
        return view('livewire.office.matrix-tree');
    }
}
